...
const { defaultServerDir } = require('../default')

class UCIManager {
  constructor(config = {}) {
    this.config = {
      uciDirectory: config.uciDirectory || defaultServerDir.uci,
      backupDirectory: config.backupDirectory || defaultServerDir.uciBackup,
      uuidMappingFile: config.uuidMappingFile || defaultServerDir.uuidMap,
  ...
  async loadUUIDMapping() {
    try {
      if (fs.existsSync(this.config.uuidMappingFile)) {
        ...
        this.logger.info(
          'No existing UUID mapping file found, creating new one',
        )
    ...
    try {
      ...
      this.logger.info(
        `Loaded UCI file: ${fileName} (${sectionsWithUUIDs.size} sections)${
          fileModified ? ' - Added UUIDs to file' : ''
        }`,
      )
    } catch (error) {
    ...
    try {
      ...
      for (const [uuid, sectionData] of fileData.sections) {
        ...
        try {
          ...
          this.logger.debug(
            `Published section ${
              sectionData.sectionName || sectionData.sectionType
            } (${uuid}) to ${topic}`,
          )
        } catch (publishError) {
      ...
      await this.publishSystemStatus(
        fileName,
        'loaded',
        `UCI file loaded successfully with ${publishedCount} sections`,
      )
    } catch (error) {
  ...
  async connectToMQTT(mqttClient) {
    ...
    try {
      await this.mqttClient.publish(
        'system/startup',
        JSON.stringify(startupMessage, null, 2),
        { retain: true },
      )
    } catch (error) {
    ...
    this.logger.info(
      `Connected to MQTT and published all UCI files. Total UUIDs: ${this.uuidMapping.size}`,
    )
  }
  ...
  async handleEditCommand(command) {
    ...
    this.logger.info(
      `Processing ${action} command for ${fileName}/${sectionName}${
        uuid ? '/' + uuid : ''
      }`,
      { requestId },
    )
    ...
    try {
      ...
      await this.publishCommandResponse(requestId, 'error', error.message, {
        fileName,
        sectionName,
        uuid,
      })
  ...
  async createSection(fileName, sectionName, values, requestId) {
    ...
    await this.mqttClient.publish(topic, JSON.stringify(sectionInfo, null, 2), {
      retain: true,
    })
  ...
  async publishCommandResponse(requestId, status, message, data = {}) {
    try {
      ...
      this.logger.debug(
        `Published command response for ${requestId}: ${status}`,
      )
    } catch (error) {
