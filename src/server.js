...
const { defaultMqttServer, defaultServerDir } = require('./default')
...
class UCIConfigurationServer {
  ...
  async initialize() {
    try {
      ...
      this.uciManager = new UCIManager({
        uciDirectory: process.env.UCI_DIR || defaultServerDir.uci,
        backupDirectory: process.env.BACKUP_DIR || defaultServerDir.uciBackup,
        uuidMappingFile:
          process.env.UUID_MAPPING_FILE || defaultServerDir.uuidMap,
      ...
      this.logger.info(
        `MQTT Server: ${this.mqttServer.config.host}:${this.mqttServer.config.port}`,
      )
      this.logger.info(
        `MQTT WebSocket: ${this.mqttServer.config.host}:${this.mqttServer.config.wsPort}`,
      )
      this.logger.info(
        `Web Server: http://${this.webServer.config.host}:${this.webServer.config.port}`,
      )
      this.logger.info(
        `UCI Files: ${this.uciManager.getStatus().filesLoaded} files, ${
          this.uciManager.getStatus().totalSections
        } sections`,
      )
  ...
  async waitForMQTTClient() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkClient = () => {
        ...
        if (waited >= maxWait) {
          reject(
            new Error('MQTT internal client failed to connect within timeout'),
          )
          return
        }
  ...
  getStatus() {
    return {
      ...
      environment: process.env.NODE_ENV || defaultMqttServer.environment,
