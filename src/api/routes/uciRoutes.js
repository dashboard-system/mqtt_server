...
router.post('/files/:fileName/:sectionName', async (req, res) => {
  try {
    ...
    const result = await uciService.createUCISection(fileName, sectionName, values);
...
router.put('/files/:fileName/:sectionName/:uuid', async (req, res) => {
  try {
    ...
    const result = await uciService.updateUCISection(fileName, sectionName, uuid, values);
    res.json(result);
  } catch (error) {
    res.status(500).json({ 
      error: error.message,
...
router.delete('/files/:fileName/:sectionName/:uuid', async (req, res) => {
  try {
    const { fileName, sectionName, uuid } = req.params;
    const result = await uciService.deleteUCISection(fileName, sectionName, uuid);
    res.json(result);
  } catch (error) {
    res.status(500).json({ 
      error: error.message,
