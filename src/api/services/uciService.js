...
class UCIService {
  ...
  async listUCIFiles() {
    try {
      ...
      const files = [...new Set(topics.map(topic => {
  ...
  async getUCISections(fileName, sectionName) {
    try {
      const sections = await this.queryMQTTData(`config/${fileName}/${sectionName}/+`, 3000);
      ...
      this.logger?.error(`Error getting UCI sections ${fileName}/${sectionName}:`, error);
      throw error;
    }
  }
  ...
  async getUCISection(fileName, sectionName, uuid) {
    try {
      ...
      this.logger?.error(`Error getting UCI section ${fileName}/${sectionName}/${uuid}:`, error);
      throw error;
    }
  }
  ...
  async createUCISection(fileName, sectionName, values) {
    try {
      ...
      this.logger?.error(`Error creating UCI section ${fileName}/${sectionName}:`, error);
      throw error;
    }
  }
  ...
  async updateUCISection(fileName, sectionName, uuid, values) {
    try {
      ...
      this.logger?.error(`Error updating UCI section ${fileName}/${sectionName}/${uuid}:`, error);
      throw error;
    }
  }
  ...
  async deleteUCISection(fileName, sectionName, uuid) {
    try {
      ...
      this.logger?.error(`Error deleting UCI section ${fileName}/${sectionName}/${uuid}:`, error);
      throw error;
    }
  }
